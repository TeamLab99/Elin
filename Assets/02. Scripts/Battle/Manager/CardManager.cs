...
public class CardManager : Singleton<CardManager>
{
    ...
    [SerializeField] EMagicType magicType;
    ...
    public DeckCard PopCard()
    {
        ...
        {
            if(magicType == EMagicType.RandomNightmare)
                SetupCardBufferNightmare();
            else if (magicType == EMagicType.RandomNormal)
                SetupCardBufferNormal();
            else
                TestCardBuffer();
        }
    ...
    void TestCardBuffer()
    {
        ...
        foreach (var deck in deckCards)
        {
            if(deck.Value.engName == magicType.ToString())
            {
                for (int i = 0; i < deck.Value.percent; i++)
                {
                    shuffleCards.Add(deck.Value);
    ...
    public void EnlargeCard(bool isEnlarge, Card card)
    {
        ...
        if (isEnlarge)
        {
            Vector3 enlargePos = BattleManager.instance.CalculateLerpPoint();
    ...
    IEnumerator UseCard()
    {
        ...
        MagicManager.instance.CallMagic(selectCard.deckCard);
        selectCard.DOKill();
    ...
    public void DisableCards()
    {
        for (var index = 0; index < myCards.Count; index++)
        {
            ...
            card.DOKill();
            Managers.Pool.Push(card.GetComponent<Poolable>());
        }
