    ...
    public class VInspectorData : ScriptableObject
    {
        ...
        public void Setup(Object target)
        {
            void buttons()
            {
                ...
                var members = target.GetType().GetMembers(maxBindingFlags).OrderBy(r => r is MethodInfo);
                ...
                foreach (var member in members)
                {
                    ...
                    if (member.GetCustomAttribute<SizeAttribute>() is SizeAttribute sizeAttribute)
                        button.size = sizeAttribute.size;

                    if (member.GetCustomAttribute<SpaceAttribute>() is SpaceAttribute spaceAttribute)
            ...
            void tabs()
            {
                var attributes = target.GetType().GetFields(maxBindingFlags).Select(r => r.GetCustomAttribute<TabAttribute>()).OfType<TabAttribute>();
            ...
            void foldouts()
            {
                var attributes = target.GetType().GetFields(maxBindingFlags).Select(r => r.GetCustomAttribute<FoldoutAttribute>()).OfType<FoldoutAttribute>();
